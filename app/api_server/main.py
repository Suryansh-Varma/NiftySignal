...
def _scheduler_enabled() -> bool:
    return os.getenv("ENABLE_BACKGROUND_SCHEDULER", "true").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
async def startup_scheduler():
    """Start background scheduler when API server starts (configurable via env)."""
    global _scheduler_instance

    if not _scheduler_enabled():
        logger.info("Background scheduler disabled via ENABLE_BACKGROUND_SCHEDULER")
        return

    if _scheduler_instance is not None:
        logger.info("Background scheduler already running in this process")
        return
    ...
    try:
        ...
        logger.info("Background scheduler started from API server startup")
    except Exception as e:
        logger.error(
            f"Failed to start background scheduler on startup: {e}", exc_info=True
        )


@app.on_event("shutdown")
async def shutdown_scheduler():
    """Shutdown background scheduler when API server stops."""
    global _scheduler_instance
    ...
    try:
        ...
        logger.info("Background scheduler stopped on API server shutdown")
    except Exception as e:
...
CSV_PATH = (
    Path(__file__).parent.parent.parent / "results" / "latest_recommendations.csv"
)
...
def _ensure_evaluation_json() -> Path:
    ...
    if not eval_json.exists():
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to evaluate strategies: {e}"
            )
    return eval_json
...
def _load_best_evaluation() -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to load evaluation JSON: {e}"
        )
...
def _read_goal_recommendations(strategy: str) -> List[dict]:
    ...
    try:
        if not csv_path:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid strategy: {strategy}. Choose from: conservative, moderate, aggressive",
            )
        ...
        raise HTTPException(
            status_code=500, detail="Goal recommendations data is corrupted"
        )
    except FileNotFoundError as e:
        logger.error(str(e))
        raise HTTPException(
            status_code=404,
            detail=f"Recommendations not found for {strategy} strategy. Please run training first.",
        )
...
def get_recommendations(
    symbol: Optional[str] = Query(
        None, description="Filter by symbol, e.g. RELIANCE.NS"
    ),
):
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail="Recommendations not found. Please run model training first.",
        )
    except HTTPException:
        raise
    except Exception as e:
...
def predict_live(symbol: str):
    ...
    try:
        ...
        if df_live.empty:
            raise HTTPException(
                status_code=404, detail=f"No live data found for {symbol}"
            )

        # 2. Prepare features
        X_live, _, _ = prepare_features(df_live, forward_days=5, return_threshold=0.015)
        if X_live.empty:
            raise HTTPException(
                status_code=400,
                detail=f"Insufficient data for {symbol} to generate features",
            )

        # 3. Load Model & Predict
        model_path = (
            Path(__file__).parent.parent.parent / "models" / "trading_model.pkl"
        )
        if not model_path.exists():
            raise HTTPException(
                status_code=500,
                detail="Model file missing. Please train the model first.",
            )
        ...
        adj_signal, adj_confidence = adjust_signal_by_risk(
            signal, confidence, macro_risk
        )
        ...
        return JSONResponse(
            content={
                ...
                "last_date": str(
                    df_live.iloc[-1]["date"].iloc[-1]
                    if hasattr(df_live.iloc[-1]["date"], "iloc")
                    else df_live.iloc[-1]["date"]
                ),
                "recommendation": signal_map.get(adj_signal, "HOLD"),
                "confidence": round(adj_confidence, 4),
                "buy_prob": round(float(proba[list(model.model.classes_).index(1)]), 4)
                if 1 in model.model.classes_
                else 0.0,
                "sell_prob": round(
                    float(proba[list(model.model.classes_).index(-1)]), 4
                )
                if -1 in model.model.classes_
                else 0.0,
...
def get_goal_recommendations(
    ...
    if symbol:
        ...
        if not filtered:
            logger.info(
                f"No goal recommendations found for symbol: {symbol} in {strategy} strategy"
            )
        return JSONResponse(content=filtered)
    return JSONResponse(content=records)
...
def refresh_risk():
    ...
    # Cooldown check
    if _last_risk_update and datetime.now() - _last_risk_update < timedelta(
        seconds=_COOLDOWN_SECS
    ):
        remaining = int(
            _COOLDOWN_SECS - (datetime.now() - _last_risk_update).total_seconds()
        )
        raise HTTPException(
            status_code=429,
            detail=(
    ...
    try:
        ...
        logger.info(
            f"On-demand risk refresh complete: {result['adjusted_risk_factor']:.4f} ({result['risk_level']})"
        )
        return JSONResponse(
            content={
                "status": "ok",
...
def refresh_recommendations():
    ...
    # Cooldown check
    if _last_recs_update and datetime.now() - _last_recs_update < timedelta(
        seconds=_COOLDOWN_SECS
    ):
        remaining = int(
            _COOLDOWN_SECS - (datetime.now() - _last_recs_update).total_seconds()
        )
        raise HTTPException(
            status_code=429,
            detail=(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Recommendation refresh failed: {e}"
        )
    finally:
        _update_lock.release()
...
def get_market_risk():
    ...
    try:
        macro_risk_file = (
            Path(__file__).parent.parent.parent / "data" / "macro_risk_factor.json"
        )
        ...
        return JSONResponse(
            content={
                "risk_score": data.get(
                    "adjusted_risk_factor", data.get("macro_risk_factor", 0.5)
                ),
...
def portfolio_optimizer(
    ...
    try:
        ...
        if len(buy_signals) == 0:
            return JSONResponse(
                content={"error": "No BUY signals available"}, status_code=404
            )
        ...
        def create_strategy(risk_level, num_stocks, max_per_stock, expected_monthly):
            ...
            selected["shares"] = (
                selected["allocation"] / selected["last_price"]
            ).astype(int)
        ...
        strategies = {
            "conservative": create_strategy(
                "conservative", min(15, len(buy_signals)), 0.10, 0.025
            ),
            "moderate": create_strategy(
                "moderate", min(10, len(buy_signals)), 0.15, 0.04
            ),
            "aggressive": create_strategy(
                "aggressive", min(7, len(buy_signals)), 0.25, 0.06
            ),
        }
