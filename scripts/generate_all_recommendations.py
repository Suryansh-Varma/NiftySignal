...
def generate_recommendations_all_stocks():
    ...
    try:
        df_all = fetch_history_nselib(
            all_symbols, start=start_date, end=end_date, include_delivery=False
        )
        logger.info(
            f"[SUCCESS] Fetched data: {df_all.shape[0]} rows, {df_all['symbol'].nunique()} symbols"
        )
    except Exception as e:
    ...
    valid_symbols = symbol_counts[
        symbol_counts >= DataValidationConfig.MIN_DATA_POINTS_PER_SYMBOL
    ].index.tolist()

    logger.info(
        f"[SUCCESS] {len(valid_symbols)}/{len(all_symbols)} stocks have sufficient data"
    )
    ...
    X_all, _, df_features = prepare_features(
        df_valid,
    ...
    try:
        ...
        logger.info(f"Model classes: {list(model.model.classes_)}")
    except Exception as e:
    ...
    feature_cols = list(model.feature_columns or X_all.columns)
    X = X_all.copy()
    ...
    df_recommendations["signal_order"] = df_recommendations["recommendation"].map(
        signal_order
    )
    df_recommendations = df_recommendations.sort_values(
        ["signal_order", "confidence"], ascending=[True, False]
    ...
    logger.info(
        f"[SUCCESS] Saved {len(df_recommendations)} recommendations to {csv_path}"
    )
    ...
    if buy_count > 0:
        ...
        top_buy = df_recommendations[
            df_recommendations["recommendation"] == "BUY"
        ].head(5)
        for idx, row in top_buy.iterrows():
            logger.info(
                f"  {row['symbol']:15} | Price: ₹{row['last_price']:10.2f} | Confidence: {row['confidence']:.1%}"
            )

    if sell_count > 0:
        logger.info(f"\nTop 5 SELL recommendations:")
        top_sell = df_recommendations[
            df_recommendations["recommendation"] == "SELL"
        ].head(5)
    ...
    logger.info(
        f"Next: Update frontend to display all {len(df_recommendations)} recommendations"
    )
