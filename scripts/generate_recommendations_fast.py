...
def main():
    ...
    try:
        df = fetch_history_nselib(all_symbols, start=start_date, end=end_date, include_delivery=False)
        logger.info(f"[SUCCESS] Fetched {len(df)} rows for {df['symbol'].nunique()} symbols")
    ...
    try:
        ...
        logger.info(f"[SUCCESS] Model loaded, using {len(feature_columns) if feature_columns else 'default'} features")
    except Exception as e:
    ...
    try:
        ...
        # Drop labels from df_feat if they were generated
        df_feat = X_all
    ...
    df_res = df_res.sort_values(['order', 'confidence'], ascending=[True, False]).drop('order', axis=1)
    ...
    if buy > 0:
        ...
        for idx, r in df_res[df_res['recommendation'] == 'BUY'].head(5).iterrows():
            logger.info(f"  {r['symbol']:15} | ₹{r['last_price']:8.2f} | {r['confidence']:.1%} confidence")
