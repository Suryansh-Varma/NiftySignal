...
print("\n" + "=" * 70)
print("RETRAINING MODEL WITH RECENT DATA (Aug 2025 - MAYs 2026)")
print("=" * 70)
...
# Filter for recent 6 months (Regime alignment)
cutoff_date = datetime.now() - timedelta(days=180)
df_recent = df[df["date"] >= cutoff_date].copy()

print(
    f"Total data available: {len(df)} rows ({df['date'].min().date()} to {df['date'].max().date()})"
)
print(
    f"Recent data (6 months): {len(df_recent)} rows ({df_recent['date'].min().date()} to {df_recent['date'].max().date()})"
)
...
try:
    X, y, df_ml = prepare_features(
        ...
        return_threshold=0.02,  # Higher threshold for cleaner labels
    )
...
try:
    ...
    for label_name in ["SELL", "HOLD", "BUY"]:
        if label_name in test_metrics["classification_report"]:
            ...
            print(
                f"  {label_name}: precision={report['precision']:.2%}, recall={report['recall']:.2%}, f1={report['f1-score']:.2%}"
            )
...
try:
    ...
    print(
        f"  Signal distribution: {jan21_df['recommendation'].value_counts().to_dict()}"
    )
except:
    jan21_df = None
...
try:
    ...
    print(
        f"  Signal distribution: {jan28_df['recommendation'].value_counts().to_dict()}"
    )
except:
    jan28_df = None
...
if not X_latest.empty:
    ...
    if latest_signals is not None and latest_proba is not None:
        ...
        try:
            ...
            recommendations.to_csv(
                recommendations_path, index=False, float_format="%.3f"
            )
