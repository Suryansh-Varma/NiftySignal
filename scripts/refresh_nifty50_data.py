...
def main():
    ...
    data = yf.download(NIFTY_50, start=start, end=end, progress=True, threads=True)
    ...
    for symbol in NIFTY_50:
        try:
            sym_data = data.xs(symbol, axis=1, level=1) if isinstance(data.columns, pd.MultiIndex) else data
    ...
    if rec_path.exists():
        ...
        print(recs[['symbol', 'last_price', 'last_date', 'recommendation']].head(10).to_string())
